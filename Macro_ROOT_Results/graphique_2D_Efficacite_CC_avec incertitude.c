...
//Double_t number_datas = 19;
	...
	//Avec Cut Si et BGO
	/*Double_t datas_LYSO [20]	= {8394, 9471, 8346, 6356, 5552, 5136, 4702, 4585, 4607, 4529, 4493, 4451, 4839, 5089, 5647, 6390, 8364, 9487, 8413} ; // valeurs en Y
	Double_t datas_BGO [20]		={13333, 20368, 23288, 23482, 22281, 20490, 18807, 17929, 17416, 17313, 17683, 17619, 18924, 20526, 22196, 23520, 23108, 20372, 13431};
	Double_t datas_LaBr3[20]	= {9445, 18301, 24931, 31245, 33388, 35296, 37258, 38002, 38008, 38384, 38232, 38021, 37205, 35570, 33471, 31061, 24808, 18343, 9598}   ;
	Double_t datas_LYSO1 [20]	=  {6144, 12638, 18884, 27231, 30670, 33715, 36735, 38710, 39135, 39267, 39583, 38691, 37380, 34377, 30562, 27051, 18893, 12580, 6128}; // valeurs en Y*/
    
    //Sans aucun Cut énergie
  ...
  //  8744, 9943, 8927, 7086, 6351, 5894, 5521, 5447, 5462, 5371, 5338, 5276, 5676, 5886, 6407, 7113, 8954, 10001, 8717
	...
	Double_t datas_BGO1 [20]	=  { 0.0234506, 0.0283499, 0.0296077, 0.0265687, 0.0221289, 0.0185885, 0.015894, 0.0138322, 0.0122311, 0.010854, 0.0097395, 0.008791, 0.0082584, 0.0080094, 0.0073364, 0.006669, 0.0060908, 0.0040215, 0.0027234, 0.0019486};
	Double_t datas_LaBr31[20]	={0.010478, 0.0140325, 0.0155404, 0.0147967, 0.0129217, 0.0112241, 0.0099016, 0.0088371, 0.008007, 0.0072724, 0.0066255, 0.006062, 0.0058294, 0.0055846, 0.005135, 0.0047572, 0.0043882, 0.0029812, 0.0020179, 0.0012559};
	...
	/*for (Int_t i=0;i<19;i++)
	{
	incertitude_300kev[i] = sqrt(datas_300keV[i]*pow(10,8))*(1/pow(10,8));
        incertitude_500kev[i] = sqrt(datas_500keV[i]*pow(10,8))*(1/pow(10,8));
        incertitude_1Mev[i] = sqrt(datas_1Mev[i]*pow(10,8))*(1/pow(10,8));
        incertitude_2Mev[i] = sqrt(datas_2Mev[i]*pow(10,8))*(1/pow(10,8));
        incertitude_4Mev[i] = sqrt(datas_4Mev[i]*pow(10,8))*(1/pow(10,8));
        incertitude_6Mev[i] = sqrt(datas_6Mev[i]*pow(10,8))*(1/pow(10,8));
		}
	*/
...
// Energy : 1.30849 strips 43
//Energy2 : 1.4723 strips 32
